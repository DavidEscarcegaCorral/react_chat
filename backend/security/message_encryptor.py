"""Cifrado híbrido: AES-128-CBC (mensaje) + RSA-OAEP SHA-256 (clave). Formato: base64(key)|base64(msg)."""
...
def encrypt_message(message: str, public_key_pem: str = None) -> str:
    """Cifra mensaje: AES-128-CBC aleatorio, clave cifrada con RSA-OAEP, retorna b64(key)|b64(msg)."""
    if public_key_pem is None:
        public_key_pem = get_server_public_key()
    aes_key = generate_aes_key()
    aes_iv = generate_aes_iv()
    encrypted_message = aes_encrypt(message.encode('utf-8'), aes_key, aes_iv)
...
def decrypt_message(encrypted_data: str, private_key_pem: str) -> str:
    """Inverso de encrypt_message: separa por |, descifra RSA, luego AES, retorna texto plano."""
    ...
    aes_key = key_iv[:16]
    aes_iv = key_iv[16:]
    return aes_decrypt(encrypted_message, aes_key, aes_iv).decode('utf-8')
